...
*** Advanced usage
	...
	Setting 'PUID' will automatically create a user with the same user id within
	the container and su's to that user.  This allows you to edit files within
	the container as if you were the user running the command.

	(There's probably a better way to do this, but this works well enough for
	me!)
